...
<form action="<?php echo JRoute::_('index.php?option=com_jux_real_estate&view=amenities'); ?>" method="post" name="adminForm"
      id="adminForm">
    <fieldset id="filter-bar">
        <div class="filter-search fltlft">
            <label class="filter-search-lbl" for="filter_search"><?php echo JText::_('JUXARCH_FILTER_LABEL'); ?></label>
            <input type="text" name="filter_search" class="inputbox" id="filter_search"
                   value="<?php echo $this->escape($this->state->get('filter.search')); ?>"
                   title="<?php echo JText::_('JUXARCH_FILTER_SUBMIT'); ?>"/>
            <button type="submit"><?php echo JText::_('JUXARCH_FILTER_SUBMIT'); ?></button>
            <button type="button"
                    onclick="document.id('filter_search').value='';document.id('filter_cat_id').selectedIndex='';this.form.submit();"><?php echo JText::_('JUXARCH_FILTER_CLEAR'); ?></button>
        </div>
        <div class="filter-select fltrt">
            <select name="filter_cat_id" id="filter_cat_id" class="inputbox" onchange="this.form.submit()">
                <?php echo JHtml::_('select.options', JFormFieldJUXAmenityCat::getOptions(true), 'value', 'text', $this->state->get('filter.cat_id'));?>
            </select>
        </div>
    </fieldset>
    <div class="clr"></div>
    ...
    <table class="adminlist" cellspacing="1">
        ...
        <tr>
            ...
            <th width="25%"><?php echo JHtml::_('grid.sort', 'COM_JUX_REAL_ESTATE_TITLE', 'title', $listDirn, $listOrder); ?></th>
            <th width="24%"><?php echo JHtml::_('grid.sort', 'COM_JUX_REAL_ESTATE_CATEGORY', 'cat', $listDirn, $listOrder); ?></th>
            <th width="1%">&nbsp;</th>
            <th width="25%"><?php echo JHtml::_('grid.sort', 'COM_JUX_REAL_ESTATE_TITLE', 'title', $listDirn, $listOrder); ?></th>
            <th width="24%"><?php echo JHtml::_('grid.sort', 'COM_JUX_REAL_ESTATE_CATEGORY', 'cat', $listDirn, $listOrder); ?></th>
        </tr>

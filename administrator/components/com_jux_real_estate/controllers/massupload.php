 ...
 * @since	1.6
 */
class JUX_Real_EstateControllerMassupload extends JControllerForm {
    ...
    public function massupload() {
        ...
        $context = "$this->option.massupload.$this->context";
    ...
    public function upload() {
        ...
        $files_realty = $this->input->post->get('realty_id');
        $responses = array();
        if (is_array($files)) {
            foreach ($files as $index => $file) {
                ...
                if (is_array($file) && isset($file['name']) && !empty($file['name'])) {
                    if ($this->_uploadFile($file)) {
                        ...
                        $model = $this->getModel();
                        if ($model->save($data)) {
                            $response->delete_url = JURI::base() . 'index.php?option=com_jux_real_estate&task=massupload.delete&id=' . $model->getState($model->getName() . '.id', '');
                        } else {

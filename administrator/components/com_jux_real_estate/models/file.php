...
class JUX_Real_EstateModelFile extends JModelAdmin {
    ...
    public function getUploadFiles($realty_id) {
        ...
        $delete_url = JURI::base() . 'index.php?option=com_jux_real_estate&task=realty.delete&id=';
    ...
    public function cleanUploadFiles() {
        // Create a new query object.
        $db = $this->getDbo();
        $query = $db->getQuery(true);

        // Select the required fields from the table.
        $query->update('#__re_files AS a');
        $query->set('a.temp_upload = 0');
        $query->where('a.temp_upload = 1');

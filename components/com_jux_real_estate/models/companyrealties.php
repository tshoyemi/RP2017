...
/**
 * JUX_Real_Estate Component - Companyrealties Model
 * @package        JUX_Real_Estate
 * @subpackage    Model
 * @since        3.0
 */
class JUX_Real_EstateModelCompanyrealties extends JModelList {
     ...
     * @var        string
     */
    protected $_context = 'com_jux_real_estate.companyrealties';
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $value = $app->getUserStateFromRequest('com_jux_real_estate.companyrealties.limit', 'limit', $limit);
...
//        parent::populateState('r.title', 'asc');
    ...
    protected function getListQuery() {
        ...
        if ($this->getState('list.company_id')) {
            $query->where('(r.company_id = ' . (int) $this->getState('list.company_id') . ' OR r.user_id IN (SELECT user_id FROM #__re_agents WHERE published = 1 AND approved = 1 AND company_id = ' . (int) $this->getState('list.company_id') . '))');
        }
...
//        }
    ...
    function getFeatured() {
        ...
        $query->select('r.*, r.id AS id, r.title AS title, r.street_num, r.street,  r.description as description, r.date_created as fcreated, co.name as company_name, r.alias as realty_alias, c.alias as cat_alias, a.alias as agent_alias, co.alias as co_alias')
                ->from('#__re_realties AS r')
                ->leftJoin('#__re_realtyamid as rm ON rm.realty_id = r.id')
                ->leftJoin('#__categories as c ON c.id = r.catid')
                ->leftJoin('#__re_types as t ON t.id = r.type_id')
                ->leftJoin('#__re_companies as co ON co.id = r.company_id')
                ->leftJoin('#__re_agents as a on a.id = r.agent_id')
        ...
        // Ordering
        $query->order('RAND()');
        $db->setQuery($query, 0, $configs->get('company_feat_num', 2));
        ...
        if (count($items)) {
            for ($i = 0; $i < count($items); $i++) {
                ...
                # Format Price and SQft output
                $items[$i]->formattedprice = JUX_Real_EstateHTML::getFormattedPrice($items[$i]->price, $items[$i]->type_freq, false, $items[$i]->call_for_price, $items[$i]->price2);
                $items[$i]->formattedsqft = number_format($items[$i]->sqft);
